...
user.friends = [...deepCopyUser.friends]
...
console.log(deepCopyStudents[2] === students[2]) //- что должно быть в консоли?
...
let bestStudent = deepCopyStudents.reduce((acc, curr) => acc.scores > curr.scores ? acc : curr)
console.log(bestStudent)

//13. Найдите сумму баллов всех студентов (reduce)
let scoresSum = deepCopyStudents.reduce((acc, currScore) => acc + currScore.scores, 0)
...
const addFriends = (students) => {
    return students.map(s => ({...s, friends: students.filter(el => el.name !== s.name).map(x => x.name)}))
}
